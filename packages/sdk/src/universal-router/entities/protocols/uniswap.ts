...
import {
  ...
  Percent,
  Protocol,
  ETH_ADDRESS_02 as ROUTER_AS_RECIPIENT,
  ETH_ADDRESS_01 as SENDER_AS_RECIPIENT,
  TradeType,
} from "@x7/utils"
...
export type SwapOptionsType = Omit<SwapOptions, "inputTokenPermit"> & {
...
export class UniswapTrade implements Command {
  ...
  constructor(
    public trade: Trade<Currency, Currency, TradeType>,
    public options: SwapOptionsType
  ) {
  ...
  encode(planner: RoutePlanner, _config: TradeConfig): void {
    ...
    let minimumAmountOut: bigint = this.trade.minimumAmountOut(
    ...
    if (routerMustCustody) {
      ...
      if (this.options.fee) {
        ...
        if (this.trade.tradeType === TradeType.EXACT_OUTPUT) {
          minimumAmountOut =
            minimumAmountOut - (minimumAmountOut * feeBips) / 10000n
        }
      }
      ...
      if (this.options.flatFee) {
        ...
        if (minimumAmountOut < BigInt(feeAmount))
        ...
        // Otherwise, we continue as expected with the trade's normal expected output
        if (this.trade.tradeType === TradeType.EXACT_OUTPUT) {
          minimumAmountOut = minimumAmountOut - BigInt(feeAmount)
        }
      }
...
function addV3Swap<TInput extends Currency, TOutput extends Currency>(
  ...
  const trade = TradeV3.createUncheckedTrade({
    route: route as RouteV3Wrapper<TInput, TOutput>,
    inputAmount,
    outputAmount,
    tradeType,
  })
...
function addMixedSwap<TInput extends Currency, TOutput extends Currency>(
  ...
  const sections = partitionMixedRouteByProtocol(
    route as MixedRoute<TInput, TOutput>
  )
...
function riskOfPartialFill(
  trade: Trade<Currency, Currency, TradeType>
): boolean {
  return trade.priceImpact.greaterThan(REFUND_ETH_PRICE_IMPACT_THRESHOLD)
}

function hasFeeOption(swapOptions: SwapOptionsType): boolean {

...
export function generateChainDenomination(chain) {
    return chainMapping[chain]?.denomination ?? ChainDenominationEnum.eth;
}
export function generateChainIdentifier(chain) {
    return chainMapping[chain]?.identifier ?? ChainIdentifierEnum.eth;
}
export function generateChainAbbreviation(chain) {
    return chainMapping[chain ?? ChainId.ETHEREUM]?.abbreviation ?? "ETH";
}
export function generateChainShortName(chain) {
    return (chainMapping[chain ?? ChainId.ETHEREUM]?.shortName ?? ChainShortNameEnum.eth);
}
export function generateChainName(chain) {
    return chainMapping[chain ?? ChainId.ETHEREUM]?.name ?? ChainNameEnum.eth;
}
export function generateChainTokenOracleEtherUSDEnum(chainId) {
    return (chainMapping[chainId ?? ChainId.ETHEREUM]?.tokenOracleEtherUSDEnum ??
        ChainTokenOracleEtherUSDEnum.eth);
}
export function getAlchemyUrls(id) {
    switch (id) {
...
//# sourceMappingURL=chainFormatters.js.map
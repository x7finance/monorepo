...
export class Graph {
    ...
    setPricesStable(from, price, networks, minLiquidity = 0, logging = false) {
        ...
        processedVert.forEach((v) => {
            ...
            console.assert(gasPriceChainId !== undefined, `Error 427: token {${v.token.address} ${v.token.symbol}}` +
                ` has unknown chainId ${v.token.chainId} (${typeof v.token
                    .chainId}).` +
                `Known chainIds: ${Array.from(gasPrice.keys()).map((k) => `"${k}"(${typeof k})`)}`);
            console.assert(v.price !== 0, `Error 428: token {${v.token.address} ${v.token.symbol} ${v.token.chainId}} was not priced`);
    ...
    getRouteLegs(from, to) {
        ...
        vertices.forEach((n) => {
            ...
            outEdges.forEach((e, i) => {
                ...
                const poolType = edge.pool instanceof StableSwapRPool
                    ? "Stable"

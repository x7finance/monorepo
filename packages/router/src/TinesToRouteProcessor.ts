...
import { getBigInt, RouteStatus } from "@x7/tines";
...
export class TinesToRouteProcessor {
  ...
  getPoolOutputAddress(
    ...
    let outAddress: string | undefined;
    ...
    if (outputDistribution.length === 0) {
      ...
      const poolCode = outputDistribution[0]
        ? this.getPoolCode(outputDistribution[0])
        : undefined;
      if (poolCode) {
        outAddress = poolCode.getStartPoint(l, route);
  ...
  codeDistributeInitial(route: MultiRoute): [string, Map<string, bigint>] {
    ...
    if (fromToken.address === "") {
      ...
      fromToken = route.legs[0]?.tokenTo ?? fromToken; // Change to wrapped Native
    }
    ...
    if (getTokenType(fromToken) === TokenType.BENTO)
      ...
      const poolAddress = route.legs[0]?.poolAddress ?? ""; // Add nullish coalescing operator
      hex.uint8(5).address(poolAddress); // wrapAndDistributeERC20Amounts

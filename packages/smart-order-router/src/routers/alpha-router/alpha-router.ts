    ...
    ISwapToRatio<AlphaRouterConfig, SwapAndAddConfig>
{
  ...
  public async route(
    ...
    const { v2GasModel, v3GasModel, mixedRouteGasModel } =
  ...
  private async getGasModels(
    ...
    const v2GasModelPromise = this.v2Supported?.includes(this.chainId)
      ? this.v2GasModelFactory
          ...
          .catch((_err) => undefined) // If v2 model throws uncaught exception, we return undefined v2 gas model, so there's a chance v3 route can go through
      : Promise.resolve(undefined)

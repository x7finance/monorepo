          ...
          <div className="relative mx-8 overflow-hidden bg-black px-6 py-6 shadow-xl sm:rounded-3xl sm:px-10 sm:pt-12 sm:pb-16 md:px-12 lg:px-20">
            <div className={cn("mx-auto max-w-2xl text-center")}>
              ...
              <h2 className="font-heading text-3xl leading-[1.1] text-black sm:text-3xl md:text-6xl dark:text-white">
                X7101 - X7105 <br /> The Constellation Tokens
              </h2>

              <p className="mt-6 text-base leading-8 text-zinc-400 sm:text-xl dark:text-zinc-500">
                {metadata.description}
              </p>
              <div className="mt-6 flex flex-col items-center gap-y-4 md:gap-y-3">
                <LinkInternal
                  prefetch={true}
                ...
                <LinkInternal
                  prefetch={true}
                  href={`/swap?token0=NATIVE&token1=${X7ContractsEnum.X7101(ChainId.ETHEREUM)}`}
                  target="_blank"
                ...
                <LinkInternal
                  prefetch={true}
                  href={`/swap?token0=NATIVE&token1=${X7ContractsEnum.X7102(ChainId.ETHEREUM)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={cn(
                    buttonVariants({
                ...
                <LinkInternal
                  prefetch={true}
                  href={`/swap?token0=NATIVE&token1=${X7ContractsEnum.X7103(ChainId.ETHEREUM)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={cn(
                    buttonVariants({
                ...
                <LinkInternal
                  prefetch={true}
                  href={`/swap?token0=NATIVE&token1=${X7ContractsEnum.X7104(ChainId.ETHEREUM)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={cn(
                    buttonVariants({
                ...
                <LinkInternal
                  prefetch={true}
                  href={`/swap?token0=NATIVE&token1=${X7ContractsEnum.X7105(ChainId.ETHEREUM)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={cn(
                    buttonVariants({

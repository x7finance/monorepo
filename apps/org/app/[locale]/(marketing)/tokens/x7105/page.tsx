...
const metadata = {
  title: "X7105",
  description:
    "X7105 is the fifth constellation token, These tokens act as collateralized reserves for the X7 Lending Pool's X7 Deposit tokens. They accumulate token value and act as a lender of last resort against X7D token issuance. They are designed to be eventually price consistent, allowing for arbitrage opportunities. This series of tokens are burned on every transaction. While continually raising its floor price - it also provides further opportunities to mint new X7Deposit tokens.",
  slug: "/tokens/x7105",
  section: "default",
}
          ...
          <div className="relative mx-8 overflow-hidden bg-black px-6 py-6 shadow-xl sm:rounded-3xl sm:px-10 sm:pb-16 sm:pt-12 md:px-12 lg:px-20">
            <div className={cn("mx-auto max-w-2xl text-center")}>
              ...
              <div className="mt-6 flex flex-col items-center gap-y-2 md:gap-x-3 md:gap-y-0">
                ...
                <Link
                  href={`${XCHANGE}/#/swap?outputCurrency=${ContractsEnum.X7105}`}
                  target="_blank"
